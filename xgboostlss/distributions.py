...
class Gaussian():
    ...
    ###
    global gradient_location_gaussian
    def gradient_location_gaussian(y: np.ndarray, location: np.ndarray, scale: np.ndarray):
    ...
    global hessian_location_gaussian
    def hessian_location_gaussian(scale: np.ndarray):
    ...
    ###
    global gradient_scale_gaussian
    def gradient_scale_gaussian(y: np.ndarray, location: np.ndarray, scale: np.ndarray):
    ...
    global hessian_scale_gaussian
    def hessian_scale_gaussian(scale: np.ndarray):
    ...
    def Dist_Objective(predt: np.ndarray, data: xgb.DMatrix):
        '''A customized objective function to train each distributional parameter
        using custom gradient and hessian.
        ...
        # Location
        grad[:, 0] = gradient_location_gaussian(y=target, location=preds_location, scale=preds_scale)
        hess[:, 0] = hessian_location_gaussian(scale=preds_scale)

        # Scale
        grad[:, 1] = gradient_scale_gaussian(y=target, location=preds_location, scale=preds_scale)
        hess[:, 1] = hessian_scale_gaussian(scale=preds_scale)
    ...
    @staticmethod
    def param_dict():
        """ Dictionary that holds the name of distributional parameter and their
        correspondig repsonse functions.
        """
